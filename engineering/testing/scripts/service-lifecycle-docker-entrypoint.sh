#!/usr/bin/env bash
# Runs inside Dockerfile.service-lifecycle-live-linux — boot systemd, then lifecycle pack.
set -euo pipefail
...
exec dbus-run-session -- bash /repo/engineering/testing/scripts/service-lifecycle-live-linux.sh
