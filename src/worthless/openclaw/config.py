...
def read_config(path: Path) -> dict[str, Any]:
    ...
    Returns ``{}`` if the file does not exist OR if the file is not readable
    by the current user (``PermissionError``). The latter happens in the
    all-container Docker setup when the openclaw daemon rewrites the shared
    config file as ``600 node:node`` after its first initialisation cycle.
    Treating permission errors as "no existing config" lets ``set_provider``
    write a fresh provider entry via an atomic replace (the shared directory
    is always ``777``, so ``os.replace`` succeeds even when the target inode
    is foreign-owned).  Non-worthless config keys (e.g. gateway auth token)
    are lost and regenerated by openclaw on next container restart — which
    is always required after a re-lock anyway.
    ...
    try:
        ...
        # File exists but is not readable (foreign-owned in Docker shared vol).
        # Return {} so callers can still write their entries via atomic replace.
        return {}
