...
class TestIsMacosKeyring:
    ...
    def test_returns_false_when_get_keyring_raises(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_returns_false_for_non_macos_backend(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_returns_true_for_macos_backend(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
...
class TestListSyncedKeychainEntries:
    ...
    def test_returns_empty_when_find_synced_entries_raises(
        ...
        with caplog.at_level(logging.DEBUG), patch.object(
            doctor_module, "_keystore_macos", km
        ):
            result = doctor_module._list_synced_keychain_entries()
...
class TestImportRecoveryFiles:
    ...
    def test_returns_zero_when_keystore_macos_is_none(
...
class TestMigrateSyncedKeysEdgeCases:
    ...
    def test_aborts_on_user_cancelled(self, fake_home: WorthlessHome) -> None:
        ...
        with patch.object(doctor_module, "_keystore_macos", km):
            result = doctor_module._migrate_synced_keys(
                ["fernet-key-X", "fernet-key-Y"], fake_home
            )
    ...
    def test_partial_success_with_non_fatal_exception(
        self, fake_home: WorthlessHome
    ) -> None:
...
class TestMigrateOneEdgeCases:
    ...
    def test_skips_recovery_file_creation_if_already_exists(
        self, fake_home: WorthlessHome
    ) -> None:
    ...
    def test_handles_keychain_not_found_on_delete_synced(
        self, fake_home: WorthlessHome
    ) -> None:
    ...
    def test_recovery_file_contains_utf8_encoded_value(
        self, fake_home: WorthlessHome
    ) -> None:
        """Recovery file bytes must be the UTF-8 encoding of the keychain value."""
        fake_home.recovery_dir.mkdir(mode=0o700, parents=True)
        km = _stub_keystore_macos()
        secret_value = "fernet-key-WOR456-test-value="
...
class TestDoctorConfirm:
    ...
    def test_yes_flag_returns_true_without_prompt(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_user_decline_returns_false_and_prints_cancelled(
        ...
        result = doctor_module._doctor_confirm(
            [_fake_orphan()], [], yes=False, console=console
        )
    ...
    def test_orphans_only_prompt_mentions_delete(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_user_accept_returns_true(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
...
class TestDoctorApply:
    ...
    def test_plural_grammar_single_migration(
        ...
        success_text = console.print_success.call_args.args[0]
        assert "entry" in success_text, (
            f"Singular should use 'entry': {success_text}"
        )
        assert "entries" not in success_text, (
            f"Singular should NOT use 'entries': {success_text}"
        )
...
class TestDoctorRunIntegration:
    def test_dry_run_with_synced_finding_prints_planned_only(
        ...
        apply_called: list = []
        monkeypatch.setattr(doctor_module, "_list_synced_keychain_entries", lambda: ["fernet-key-X"])
...
class TestRaiseForStatus:
    ...
    def test_keychain_error_repr_does_not_include_password(self) -> None:
        ...
        r = repr(err)
        assert "test message" not in r, (
            "KeychainError.__repr__ must NOT include the msg string"
        )
...
def test_multi_device_warning_contains_required_substrings() -> None:
    ...
    for fragment in required:
        assert fragment in warning, (
            f"_MULTI_DEVICE_WARNING missing required fragment '{fragment}'"
        )
