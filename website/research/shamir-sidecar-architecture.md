...
But the protocol still requires **Oblivious Transfer** for the evaluator's input bits. The evaluator needs to learn labels for its 400 input bits without revealing which bits it holds. This requires 128 base oblivious transfers (public-key operations costing ~10–20ms) plus Oblivious Transfer extension for the remaining 272 (~0.1ms using IKNP-style extension with AES-NI). With base oblivious transfers precomputed and cached, the amortized per-request cost drops to **<1ms on localhost**. Major frameworks supporting Free XOR include **MP-SPDZ** (~1.5K GitHub stars, C++, MIT-like, actively maintained), **EMP-toolkit** (~700 stars, C++, MIT), and **Swanky** by Galois (~123 stars, Rust, MIT — but explicitly marked "research software, do not deploy in production").
...
**Distribution:** The sidecar ships as a Rust binary inside platform-specific PyPI wheels built with maturin. `pip install worthless` installs the Python proxy and the native sidecar binary into the virtualenv. First run auto-generates shards and configures the keychain entry. No cryptographic configuration. No manual key management. The user experience is: `pip install worthless`, `worthless add-key <provider-api-key>`, `worthless start` — identical to the current flow, but with the master key file eliminated and the API key never existing as a whole in the Python process.
