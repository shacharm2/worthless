...
|---|---|---|
| **macOS** (Apple Silicon or Intel) | [macOS guide](/install/mac/) | ~2 min on a fast network |
| **Linux** (Ubuntu / Debian / Alpine) | [Linux guide](/install/linux/) | ~1-2 min on a fast network |
| **Windows + WSL2** † | [WSL2 guide](/install/wsl/) | ~2-3 min on a fast network |
| **Docker** (your app runs in a container) | [Docker guide](/install/docker/) | ~5 min (one-time per project) |

> **† WSL2 is not exercised in CI.** The underlying install is identical to
> Linux (which *is* CI-tested), but the WSL-specific layer — localhost
> forwarding, file-backed keystore, `/mnt/c` detection — is verified by hand,
> not automated. macOS, Linux, and Docker installs all run in CI.
...
> [Docker guide](/install/docker/) for the full explanation.
...
|---|---|
| Proxy doesn't auto-restart on reboot unless you run `worthless service install` | Fixed by `worthless service install` (macOS launchd / Linux systemd) |
...
| `uv tool uninstall worthless` only removes the binary — run `worthless uninstall` first to restore your keys, then purge `~/.worthless/` + the keychain | see [Uninstall](/uninstall/) |
| No `worthless` CLI is exposed by `docker run ghcr.io/.../worthless` — you still install natively | by design; see [Docker guide](/install/docker/) |
...
[agent-schema](/install/agent-schema/) — each platform guide's
